...
export default function ConsultationForm({
  ...
  const handleChange = (e) => {
    ...
    if (name === 'mobile') {
      safeValue = formatPhoneAsYouType(value);
      if (mobileError) {
        setMobileError('');
      }
    }
  ...
  const validateMobile = () => {
    const validation = validatePhoneNumber(formData.mobile);
        ...
        <div className='flex flex-col md:flex-row gap-4'>
          <div className='flex-1 relative'>
            <input
              ...
              className={`w-full px-4 py-3 rounded-md text-tertiary placeholder:text-tertiary focus:outline-none ${
                mobileError
                  ? 'bg-tertiary/20 ring-2 ring-red-400 border border-red-400'
                  : 'bg-tertiary/20'
              }`}
              style={{ minWidth: 0 }}
            />
            {mobileError && (
              <p
                className='absolute left-0 text-xs mt-1 text-red-500'
